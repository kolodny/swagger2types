...
export const generate = async (spec: Spec, params?: Params) => {
  ...
  const allExports = allContent.matchAll(/^export\s+\S+\s+(\S+)/gm);
  const allExportsNames = Array.from(allExports).map((m) => m[1]);
  const aliased = allExportsNames.map((n) => `${n} as ಠ_ಠ_${n}`);
  const aliasedJoined = aliased.join(',\n  ');

  const reason = `// The following are only exported to avoid the error: Exported variable 'X' has or is using name 'Y' from external module "Z" but cannot be named.ts(4023)`;
